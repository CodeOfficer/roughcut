...
> **Integration Point**: Use these API methods with Playwright's `page.evaluate()` to control presentation during video recording
...
> **Timing Note**: Use `Reveal.next()` and fragment navigation for synchronized playback with your audio timeline
...
> **Automation Note**: Use `Reveal.getProgress()` to track recording progress
...
> **Video Recording**: Use `Reveal.togglePause()` to control recording state if needed
...
- [Plugin API](./36-using-plugins.md#api)

---

**For Your Project**: These API methods are essential for Playwright automation. Key methods:
- `Reveal.next()` - Advance slides during recording
- `Reveal.nextFragment()` - Reveal fragments step-by-step
- `Reveal.getProgress()` - Track progress for synchronization
- `Reveal.getCurrentSlide()` - Get current slide for state management
