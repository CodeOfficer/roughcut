...
function checkWorkspace(): CheckResult {
  ...
  if (wsRoot) {
    const hasEnv = existsSync(join(wsRoot, ".env"));
    const result: CheckResult = {
      name: "workspace",
      status: "ok",
      detail: `Workspace found at ${wsRoot}`,
    };
    if (!hasEnv) {
      result.hint = "No .env file found — create one to store API keys";
    }
    return result;
  }
  return {
    name: "workspace",
    status: "warn",
    detail: "Not inside a roughcut workspace",
    hint: 'Run "roughcut init" to create a workspace with shared config and .env',
  };
}
...
function checkFfmpeg(): CheckResult {
  try {
    const output = execSync("ffmpeg -version 2>&1", {
      encoding: "utf-8",
      timeout: 5000,
    });
...
function checkApiKey(
  ...
  return {
    ...
    hint: `Set in .env, environment, or ~/.config/roughcut/config.yml`,
  };
}
