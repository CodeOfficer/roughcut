...
export function createVoicesCommand(): Command {
  ...
  cmd.description("List available ElevenLabs voices").action(async () => {
    try {
      await runVoices();
    } catch (error) {
...
async function runVoices(): Promise<void> {
  ...
  console.log("Usage:");
  console.log('  In .roughcut/config.yml:   elevenlabs_voice_id: "<voice_id>"');
