...
> = {
  ...
  controlsBackArrows: { type: 'string', example: '"faded", "hidden", or "visible"' },
  progress: { type: 'boolean', example: 'true' },
  slideNumber: { type: 'boolean | string', example: 'false, true, "h.v", "h/v", "c", "c/t"' },
  ...
  // Transitions
  transition: { type: 'string', example: '"none", "fade", "slide", "convex", "concave", "zoom"' },
 ...
 */
function findClosestMatch(input: string, validOptions: string[]): string | undefined {
...
function validateField(field: string, value: unknown): ValidationError | null {
  ...
  const typeValidator = TYPE_VALIDATORS[validator.type] || (baseType ? TYPE_VALIDATORS[baseType] : undefined);
...
export function validatePreset(preset: string): ValidationResult {
  if (!isValidPreset(preset)) {
    const validPresets = ['video-recording', 'manual-presentation', 'auto-demo', 'speaker-mode'];
    ...
    const error: ValidationError = {
      ...
      example: '"video-recording", "manual-presentation", "auto-demo", or "speaker-mode"',
    };
 ...
 */
export function validateConfig(config: Partial<RevealJSConfig>): ValidationResult {
...
export function formatValidationErrors(errors: ValidationError[]): string {
  ...
  lines.push('================================================================================');
  lines.push('');
  lines.push('See docs/architecture/revealjs/CONFIGURATION.md for complete reference.');
  lines.push('');
